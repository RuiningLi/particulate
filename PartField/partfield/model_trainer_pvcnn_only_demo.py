    ...
    def predict_step(self, batch, batch_idx):
        ...
        if self.cfg.is_pc:
            ...
            if use_cuda_version:
                ...
                combined_feat = np.load("/scratch/shared/beegfs/ruining/projects/PartField/exp_results/partfield_features/correspondence/combined.npy")
                combined_feat_scaled = combined_feat / np.linalg.norm(combined_feat, axis=-1, keepdims=True)
                data_scaled = point_feat / np.linalg.norm(point_feat, axis=-1, keepdims=True)

                pca = PCA(n_components=3).fit(combined_feat_scaled)
